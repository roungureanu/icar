...
class FunctionTest(unittest.TestCase):
    def setUp(self):
        ...
        self.valid_columns = [self.valid_column, 'COLUMN2']
    ...
    def test_params_ok(self):
        ...
        icar.core.database_operations.create_table(\
            ...
            self.valid_sizes)
        output = icar.core.database_operations.remove_column(\
            self.valid_database, self.valid_table, self.valid_column)
        self.assertEqual(output, (0, 'Success: Column removed.'))

    def test_invalid_database(self):
        output = icar.core.database_operations.remove_column(\
            self.invalid_database, self.valid_table, self.valid_column)
        self.assertEqual(output, (1, 'Error: Invalid database name.'))

    def test_invalid_table(self):
        output = icar.core.database_operations.remove_column(\
            self.valid_database, self.invalid_table, self.valid_column)
    ...
    def test_non_existing_database(self):
        ...
        output = icar.core.database_operations.remove_column(\
            self.non_existing_database, self.valid_table, self.valid_column)
    ...
    def test_non_existing_table(self):
        ...
        icar.core.database_operations.create_database(self.valid_database)
        output = icar.core.database_operations.remove_column(\
            self.valid_database, self.non_existing_table, self.valid_column)
